...
Powołuj do: skanowania repo, inwentaryzacji, grepowania, czytania logów, audytu, review, separowalnych analiz oraz - jako wykonawcę - do pisania kodu w zadaniu wydzielonym przez sesję główną (skill `subagent-driven-development`).

**Subagent-wykonawca pisze wyłącznie w zakresie wyznaczonym przez sesję główną.** Nie scala, nie wypycha zmian do zdalnego repo, nie rozszerza zakresu i nie zatwierdza sam siebie - jego raport „gotowe" jest deklaracją do sprawdzenia, nie wynikiem. Zapisać zmiany lokalnie (commit w swojej gałęzi albo kopii roboczej) wolno, bo recenzenci potrzebują różnicy do oceny. Przyjęcie wyniku, integracja, weryfikacja prawdziwymi komendami (skill `dowod-przed-deklaracja`) i decyzja należą wyłącznie do sesji głównej. Subagenci audytowi i recenzenci zostają tylko do czytania.

**Izolacja: w jednym katalogu roboczym pisze naraz jeden subagent.** Równoległe pisanie jest dozwolone wyłącznie w osobnych git worktree (np. `isolation: worktree` w konfiguracji subagenta). Dwóch wykonawców w tym samym katalogu to gwarantowany konflikt, nawet gdy „dotykają innych plików".

**Wołaj gotowych agentów po nazwie kanonicznej** (`plugins/artur/agents/<nazwa>.md`), nie improwizuj od zera zestaw ról, który już istnieje jako agent (N-06). Dziś: `architecture-auditor`, `docs-memory-auditor`, `product-owner`, `security-reliability-auditor`, `testing-auditor`, `task-result-verifier`, `tooling-auditor`, `ui-design-auditor`, `ux-content-auditor`.
...
Nie pozwalaj wielu agentom równolegle pisać w tym samym katalogu roboczym - patrz reguła izolacji wyżej.

**Kolejność w poleceniu dla subagenta: stała część na górze, zmienne zadanie na dole.** Stałe zasady (rola, granice, format raportu) zostają identyczne między wywołaniami i idą pierwsze; konkretne zadanie, pliki, zakres commitów i raport poprzednika - na końcu. Tak działa cache promptu: trafia tylko niezmieniony początek (dokumentacja prompt caching na platform.claude.com). Dotyczy plików agentów, szablonów w skillach i każdego polecenia pisanego ad hoc. Subagent nie wywoła komendy ukośnikowej - instrukcję dajesz wprost, nigdy odwołanie do komendy.

**Poziom wysiłku subagentów: dziedziczony z sesji.** Konfiguracja agenta ma pole `effort` (`low` … `max`), ale oficjalna dokumentacja Claude Code nie rekomenduje poziomów dla konkretnych ról - podaje tylko ogólnie, że niższy wysiłek jest szybszy i tańszy przy prostych zadaniach (stan na 2026-09-23). Dopóki nie będzie rekomendacji albo danych z telemetrii, agentom go nie ustawiamy.
